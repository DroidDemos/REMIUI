...
public class MenuBuilder implements Menu {
    ...
    void setExclusiveItemChecked(MenuItem item) {
        ...
        while (i$.hasNext()) {
            MenuItem curItem = (MenuItemImpl) i$.next();
    ...
    public void setGroupCheckable(int group, boolean checkable, boolean exclusive) {
        Iterator i$ = this.mItems.iterator();
        while (i$.hasNext()) {
            MenuItemImpl item = (MenuItemImpl) i$.next();
    ...
    private static int getOrdering(int categoryOrder) {
        int index = (CATEGORY_MASK & categoryOrder) >> CATEGORY_SHIFT;
        if (index >= 0 && index < sCategoryToOrder.length) {
            return (sCategoryToOrder[index] << CATEGORY_SHIFT) | (USER_MASK & categoryOrder);
        }
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!(shortcutsVisible && this.mResources.getConfiguration().keyboard != 1 && this.mResources.getBoolean(R.bool.passport_abc_config_showMenuShortcutsWhenKeyboardPresent))) {
            z = false;
        }
        ...
        if (event.getKeyData(possibleChars) || keyCode == 67) {
            ...
            while (i$.hasNext()) {
                ...
                if ((metaState & 5) == 0 && shortcutChar != '\u0000') {
                    if ((shortcutChar == possibleChars.meta[0] || shortcutChar == possibleChars.meta[2] || (qwerty && shortcutChar == '\b' && keyCode == 67)) && item.isEnabled()) {
                        items.add(item);
                    }
                }
            }
